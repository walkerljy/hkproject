...
class SequenceDataset(Dataset):
    def __init__(self, csv_file, seq_length=7, pred_steps=1):
        """
        Args:
            csv_file: 数据文件路径
        ...
        self.samples = []
        self.targets = []
        wholelength = seq_length+pred_steps
        # 将数据按seq_length分组处理
        data = raw_data.values
        for i in range(round(len(data)/wholelength)-1):
            # 输入序列：seq_length个时间步
            sample = data[i*wholelength:i*wholelength+seq_length-pred_steps]
            # 输出目标：后续pred_steps个时间步
            target = data[i*wholelength+seq_length-pred_steps:i*wholelength+wholelength]
...
for epoch in range(config["num_epochs"]):
    ...
    for inputs, targets in train_loader:
        ...
        targets = targets.to(device).squeeze(1)  # 调整维度
