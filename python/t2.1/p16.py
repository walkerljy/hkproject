...
config = {
    ...
    "num_features": 7,             # 特征列数
    "hidden_size": 32,             # 隐藏层大小（平衡容量与稳定性）
    "num_layers": 1,               # 减少层数提高稳定性
    "batch_size": 16,
    "epochs": 500,                 # 增加训练轮次确保收敛
    "learning_rate": 0.0005,       # 较低学习率
...
def train_model():
    ...
    for epoch in range(config["epochs"]):
        ...
        for batch in train_loader:
            ...
            seq_last = torch.tensor([s[2] for s in stats], dtype=torch.float32).to(device)
