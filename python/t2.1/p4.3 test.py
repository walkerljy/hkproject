...
# criterion = RelativeMSELoss()# 关键修改点
# optimizer = optim.Adam(model.parameters(), lr=config["learning_rate"])

# # 修改后的训练循环
# best_train_loss = float('inf')
# best_model_path = "python/t2.1/best_sequence_predictor.pth"
...
#         print(f"Train SSE: {train_loss:.3f} | Avg per feature: {avg_train:.5f}")
#         print(f"Test SSE: {test_loss:.3f} | Avg per feature: {avg_test:.5f}")
...
#         # 打擂台：记录最优模型
#     if train_loss < best_train_loss and epoch>=config["num_epochs"]//2:
#         best_train_loss = train_loss
#         torch.save(model.state_dict(), best_model_path)
#         print(f"✅ New best train loss: {best_train_loss:.5f} - model saved.")

# model = SequencePredictor().to(device)
# model.load_state_dict(torch.load(best_model_path))
# torch.save(model.state_dict(), "python/t2.1/sequence_predictor_4.3.pth")
